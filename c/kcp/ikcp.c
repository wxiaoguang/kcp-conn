...
const IUINT32 IKCP_CMD_CONNECT = 80;    // cmd: connect
...
const IUINT32 IKCP_STATE_CONNECTED        = 1 << 0;
const IUINT32 IKCP_STATE_REMOTE_CLOSED    = 1 << 1;
const IUINT32 IKCP_STATE_LOCAL_CLOSED     = 1 << 2;
const IUINT32 IKCP_STATE_DEAD             = 1 << 3;
...
// redefine allocator
void ikcp_set_allocator(void *(*new_malloc)(size_t), void (*new_free)(void *))
{
...
//---------------------------------------------------------------------
void ikcp_set_output(ikcpcb *kcp, int (*output)(const char *buf, int len,
...
int ikcp_send(ikcpcb *kcp, const char *buffer, int len)
{
	...
	//how many segments we need. we have checked that len is not zero, so count must be at least 1
...
void ikcp_send_connect_flush(ikcpcb *kcp)
{
	IKCPSEG *seg;

	if(ikcp_waitsnd(kcp) == 0 && kcp->snd_nxt == 0) {
		seg = ikcp_segment_new(kcp, 0);
...
void ikcp_send_close_flush(ikcpcb *kcp)
{
	IKCPSEG *seg;

	if(ikcp_state_connected(kcp) && !ikcp_state_local_closed(kcp)) {
...
static inline IUINT32 ikcp_calc_cwnd(ikcpcb *kcp)
{
...
int ikcp_sndbuf_avail(ikcpcb *kcp)
{
	IUINT32 cwnd = ikcp_calc_cwnd(kcp);
...
int ikcp_set_mtu(ikcpcb *kcp, int mtu)
{
	char *buffer;
...
int ikcp_set_nodelay(ikcpcb *kcp, int nodelay, int interval, int resend, int nc)
{
	if (nodelay >= 0) {
		kcp->nodelay = nodelay;
		if (nodelay) {
...
int ikcp_set_wnd(ikcpcb *kcp, int sndwnd, int rcvwnd)
{
	if (kcp) {
		if (sndwnd > 0) {
			kcp->snd_wnd = sndwnd;
...
// read conv
IUINT32 ikcp_get_conv(const void *ptr)
{
	IUINT32 conv;
